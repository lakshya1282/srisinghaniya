    ...
    >
      <div className="max-w-[1650px] mx-auto px-4 sm:px-8 lg:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-stretch">
          ...
          {/* Left Column: Content, Grid, and Button */}
          <div className="lg:col-span-7 flex flex-col justify-between">
            ...
            <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
              {advantages.map((item) => (
                <div
                  key={item.id}
                  className="p-8 rounded-2xl border border-zinc-200/60 bg-white shadow-md hover:shadow-lg transition-all duration-500 select-none"
                  style={{
                    opacity: isVisible ? 1 : 0,
                    transform: isVisible ? "translateY(0)" : "translateY(20px)",
                    transitionDelay: `${parseInt(item.id) * 150}ms`,
                  }}
                >
                  <div className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-zinc-100 text-xs font-bold text-regal-navy shrink-0">
                      {item.id}
                    </span>
                    <h3 className="text-lg font-bold text-zinc-950 font-sans leading-snug">
                      {item.title}
                    </h3>
                  </div>
                  <p className="text-sm font-medium text-zinc-600 mt-4 leading-relaxed font-sans">
                    {item.desc}
                  </p>
                </div>
              ))}
          ...
          >
            <ParallaxImage
              src="/images/substationdesktop.webp"
              alt="Substation Structure"
